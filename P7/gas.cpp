...
void Flatulan::move()
{
	...
	m_city->determineNewPosition(temprow, tempcol, dir);
	if (temprow != m_city->player()->row() || tempcol != m_city->player()->col())
...
void Player::move(int dir)
{
	...
	if (UP == dir)
	{
		if (row() >= 2)
		{
			if (0 == m_city->nFlatulansAt(row() -1, col()))
			m_row--;
		}
	}
	if (DOWN == dir)
	...
	if (LEFT == dir)
	{
		if (col() >= 2)
		{
			if (0 == m_city->nFlatulansAt(row(), col() - 1))
				m_col--;
		}
	}
	if (RIGHT == dir)
...
void City::preachToFlatulansAroundPlayer()
{
	...
	for (int i = 0; i < m_nFlatulans; i++)
	{
			if ((m_flatulans[i]->row() == (m_player->row() - 1) && m_flatulans[i]->col() == (m_player->col() - 1)) ||
				(m_flatulans[i]->row() == (m_player->row() - 1) && m_flatulans[i]->col() == m_player->col()) ||
				(m_flatulans[i]->row() == (m_player->row() - 1) && m_flatulans[i]->col() == (m_player->col() + 1)) ||
				(m_flatulans[i]->row() == m_player->row() && m_flatulans[i]->col() == (m_player->col() - 1)) ||
				(m_flatulans[i]->row() == m_player->row() && m_flatulans[i]->col() == (m_player->col() + 1)) ||
				(m_flatulans[i]->row() == (m_player->row() + 1) && m_flatulans[i]->col() == (m_player->col() - 1)) ||
				(m_flatulans[i]->row() == (m_player->row() + 1) && m_flatulans[i]->col() == m_player->col()) ||
				(m_flatulans[i]->row() == (m_player->row() + 1) && m_flatulans[i]->col() == (m_player->col() + 1)))
